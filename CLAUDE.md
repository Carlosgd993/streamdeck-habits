...
| Cada cuánto se corrige ese tiempo con un refetch real | `config.py` → `TIMER_SYNC_SECONDS` (60s) | `orchestrator._timer_sync`: dispara `refresh_cycle()` completo, mismo gate que `_timer_tick` — ver [Vista "Cronómetros"](#vista-cronómetros) |
...
| El deck se queda sin responder un rato (sobre todo con un cronómetro corriendo) | `orchestrator.refresh_cycle` | Las ocho lecturas de red van SIN `screen_lock` a propósito — si alguna vuelve a acabar bajo el lock, cualquier tecla se congela mientras dura la red. Ver [El bucle principal](#el-bucle-principal) |
...
Como "Hoy"/"Tareas"/"Crear", pagina de cero cada ciclo (`core.screens._flat_page_builder`, sin el mapeo persistido de "Hábitos" — ver [Reparto de teclas](#reparto-de-teclas)), ordenados por `(order, id)`. A diferencia de "Hoy", nada desaparece nunca de la lista (no hay `is_done` que filtre), así que en la práctica cada log mantiene la misma tecla mientras no cambien los logs que hay, sin necesitar la persistencia de `core.key_map`.

### Vista "Mornings"

Mismo comportamiento exacto que "Hábitos" (se ven todos los hábitos con objetivo, hechos hoy en gris o pendientes en blanco, y pulsar un booleano ya hecho lo **deshace** — `core.screens.VIEWS["mornings"].allows_undo = True`), pero filtrada a solo los hábitos cuya sección (`habits.section_id` → `habit_sections.name` en `../habits-core`) es `"Mornings"`. No es una vista nueva de verdad en el sentido de "otro tipo de dato": son los mismos `Habit` de siempre (`HabitProvider.get_habits()`), solo con un filtro más en `core.screens._mornings_items` — comparación de `Habit.section_name` insensible a mayúsculas/minúsculas contra `core.screens._MORNINGS_SECTION_NAME`.

`section_name` es un campo de `provider.base.Habit` (no solo de `BooleanHabit`/`RealHabit`, para que esta vista pueda leerlo sin `isinstance`, igual que `manual_entry`) que llega ya resuelto por la base: `v_today_habits` en `../habits-core` hace `left join habit_sections` y expone `section_name` (el nombre, no el UUID crudo de `section_id`) — así el cliente no tiene que resolver el id por su cuenta. `LogHabit` no lo tiene: esta vista, como "Hábitos", solo trabaja con hábitos con objetivo.

**Qué hábitos caen en "Mornings" no se configura en el daemon**: es la columna `habits.section_id` apuntando a la fila de `habit_sections` con `name = 'Mornings'` en `../habits-core` — se crea y se asigna a mano por SQL, igual que `task_templates.show_in_deck`/`timer_labels.show_in_deck`. Si esa sección no existe todavía en un proyecto Supabase dado (p.ej. recién creado), la vista simplemente sale vacía, no falla.

A diferencia de "Hábitos", esta vista usa `core.screens._flat_page_builder` (no `_tiered_page_builder`): no reutiliza el mapeo persistido de `core.key_map` — un hábito de "Mornings" no comparte tecla con su entrada en "Hábitos", cada vista pagina de cero. Como ningún hábito desaparece de la lista solo por completarse (queda en gris, no se filtra), la tecla de cada uno es en la práctica estable entre ciclos sin necesitar esa persistencia — mismo razonamiento que "Logs"/"Cronómetros".
...
- `_timer_sync` (`config.TIMER_SYNC_SECONDS`, 60s) dispara un `refresh_cycle()` completo (refetch real de las siete lecturas + repintado) para corregir esa cuenta local — deriva de reloj, o que otro cliente haya parado/arrancado el cronómetro entre medias — sin esperar a los 15 min de `REFRESH_SECONDS`.
...
**Las ocho lecturas de `refresh_cycle` se hacen SIN sujetar `screen_lock`** — solo aplicar sus resultados y repintar, al final, lo hace (ver más abajo). No siempre fue así: hasta que se detectó en producción, las ocho corrían bajo el lock, y `on_key_change` necesita ese mismo lock para resolver cualquier pulsación — con una petición hasta 10s de timeout (`provider/supabase.py`) y `TIMER_SYNC_SECONDS` (60s) disparando el ciclo completo cada minuto mientras un cronómetro seguía corriendo, una red lenta dejaba el deck entero sin responder durante ese rato, cada minuto. Se descubrió con un timeout real de 10s en `get_daily_totals` y un fallo de DNS en `get_log_habits` en el mismo arranque. La corrección: cada lectura guarda su resultado (o su código de error) en una variable local fuera del lock; solo aplicarlas a `mapping`/cada `*_ref`/cada `last_*_code` y repintar, al final, sujeta `screen_lock` — eso sí es síncrono y rápido (sin red), así que apenas compite con una pulsación.
...
2. `get_log_habits()` — los hábitos de solo registro (`LogHabit`, ver [Vista "Logs"](#vista-logs)). Éxito → `log_habits_ref` y se limpia `last_log_habits_code`; mismo tratamiento de fallo. **No** llama a `key_map.update_mapping()`: "Logs" no reutiliza el mapeo persistido (ver [Reparto de teclas](#reparto-de-teclas)).
3. `get_tasks()` — otra sola petición; solo pendientes y ya ordenadas. Éxito → se guardan en `tasks_ref` y se limpia el código de error de tareas; mismo tratamiento de fallo que arriba con `last_tasks_code`. Las tareas no tienen mapeo propio: cada vista las reparte en tecla cuando le toca pintarse (ver [Reparto de teclas](#reparto-de-teclas)).
4. `get_templates()` — solo las de creación rápida, ya ordenadas por título. Éxito → `templates_ref` y se limpia `last_templates_code`; mismo tratamiento de fallo. Cambian rarísimamente, pero se releen cada ciclo por simetría: una petición cada 15 min no compensa un camino especial.
5. `get_timer_labels()` — las etiquetas rápidas de cronómetro (ver [Vista "Cronómetros"](#vista-cronómetros)). Éxito → `timer_labels_ref` y se limpia `last_timer_labels_code`; mismo tratamiento de fallo. **Sí** pinta rojo si falla: sin catálogo no hay nada que ofrecer.
6. `get_running_timer()` — el cronómetro en marcha ahora mismo, o `None`. Éxito → `running_timer_ref["value"]` y se limpia `last_running_timer_code`; mismo tratamiento de fallo, **pero un fallo aquí NO pinta tecla en rojo**: solo se pierde el resaltado de "cuál está corriendo", el toggle sigue siendo correcto porque lo decide `rpc/timer_toggle` contra el estado real, no contra este valor cacheado.
7. `get_daily_totals()` — segundos acumulados hoy por id de tarea/etiqueta (`v_timer_daily_totals`, ver [Vista "Cronómetros"](#vista-cronómetros)). Éxito → `daily_totals_ref["value"]` y se limpia `last_daily_totals_code`; mismo tratamiento de fallo que `get_running_timer()`: **tampoco pinta tecla en rojo**, solo se pierde el total del día hasta el próximo ciclo con éxito (la tecla sigue mostrando el nombre solo, nunca un número obsoleto).
8. `get_task_totals()` — segundos acumulados de SIEMPRE por `task_id` (`v_task_timer_totals`, ver [Vista "Cronómetros"](#vista-cronómetros)). Éxito → `task_totals_ref["value"]` y se limpia `last_task_totals_code`; mismo tratamiento de fallo que `get_daily_totals()`: **tampoco pinta tecla en rojo**, solo se pierde el acumulado histórico hasta el próximo ciclo con éxito.
9. **Ya bajo `screen_lock`**: se aplican los ocho resultados anteriores tal cual (`mapping`, cada `*_ref`, cada `last_*_code`, en el mismo orden que arriba) y `_paint_current_screen()` resuelve la **pantalla activa** (no solo "Hoy": lo que sea que esté abierto — menú, Sistema o cualquier vista) contra los datos que acaban de llegar (`core.screens.resolve_page`), la pinta (`renderer.render_page`), pinta encima los códigos de error guardados **si la pantalla visible los usa** (`last_habits_code` solo en "Hoy"/"Hábitos", `last_log_habits_code` solo en "Logs", `last_tasks_code` en "Hoy"/"Tareas", `last_templates_code` solo en "Crear", `last_timer_labels_code` solo en "Cronómetros"), y **re-registra** `deck.set_key_callback(...)` con un closure fresco.

**Las ocho lecturas fallan por separado**: la que falla no toca su mapeo ni sus datos (se conservan los del ciclo anterior) y no afecta a las otras. El ciclo **siempre** llega a pintar y a re-registrar el callback, de modo que lo pintado y lo que hace cada tecla nunca se desincronizan.

**Reactivación de un proyecto Supabase pausado.** Un proyecto del plan gratuito se pausa solo tras ~1 semana sin tráfico: su subdominio de API deja de resolver por DNS, lo que `provider.supabase` traduce en el mismo `ProviderNetworkError`/`NET` que "la Pi no tiene red" — son indistinguibles desde el propio checkin. Al final de `refresh_cycle` (ya **fuera** de `screen_lock`, es una llamada de red que no toca pantalla ni mapeo), si cualquiera de las ocho lecturas trajo `NET`, se lanza en su propio hilo `provider.keepalive.try_restore_active_project()`, que pide la reactivación a la Management API de Supabase (`api.supabase.com`, no PostgREST). Es una **credencial y una API distintas** de las que usa `provider/supabase.py`: requiere `SUPABASE_ACCESS_TOKEN` (Personal Access Token de la cuenta, no la clave publishable) en el `.env` — sin él, la función no hace nada y el daemon se comporta exactamente igual que antes de que este módulo existiera. `config.RESTORE_COOLDOWN_SECONDS` (30 min) evita repetir la petición mientras el proyecto sigue "despertando" (tarda uno o dos minutos; se confirma solo, en un ciclo posterior).
...
"Hoy", "Tareas", "Crear", "Logs" y "Cronómetros" no reservan nada para hábitos ni persisten tecla: paginan de cero, cada vez, la lista completa que les toque desde la tecla 1 en adelante (`core.screens._flat_page_builder`) — en "Hoy" primero los hábitos aún no completados (por `order`), luego las tareas pendientes (por prioridad/fecha); en "Tareas", solo las tareas; en "Crear", solo las plantillas (por título); en "Logs", todos los `LogHabit` (por `order`, ver [Vista "Logs"](#vista-logs)); en "Cronómetros", todas las `timer_labels` con `show_in_deck` (por `order`, ver [Vista "Cronómetros"](#vista-cronómetros)). Por eso ninguna deja hueco al completar algo: todo se recalcula y recoloca desde el principio — salvo "Logs" y "Cronómetros", donde nada desaparece nunca (salvo un archivado, fuera del alcance del deck), así que en la práctica la tecla de cada una SÍ es estable entre ciclos, sin necesitar el mapeo persistido de "Hábitos".
...
- `checkin_failures.log` — JSON-lines de escrituras fallidas hacia Supabase: checkins de hábito, cierres de tarea, creaciones desde plantilla y toggles de cronómetro (errores de API, tecla en rojo); el campo `kind` distingue `"habit"` / `"task"` / `"template"` / `"timer"`
