...
- **`base.py`** — los **puertos**. Contiene todo lo que el resto del proyecto necesita saber de "la API", sin acoplarse a ningún backend:
  ...
  - **`TimerLabel`** — la etiqueta rápida de cronómetro, mismo papel que `Template` pero para cronómetros: `id`, `name`, `emoji`, `order`, y `running`/`started_at` — **mutables, y no vienen del backend**: los calcula `core.screens._timer_items` cruzando etiquetas con `RunningTimer`, igual que `Template.has_pending`. `display_label()` devuelve el nombre recortado; el tiempo transcurrido lo añade `deck.renderer.render_timer`, no esta clase.
...
- **Parado, recordando el último usado**: gris (`COLOR_TIMER_SHORTCUT_IDLE`, no el turquesa de "Cronómetros" — aquí no significa "listo para arrancar", sino "en espera de que lo reactives") con su título — pulsarla lo vuelve a arrancar.
- **Sin ningún cronómetro usado todavía** (o el que se recordaba ya no existe): mismo gris, con el aviso "Sin cronómetro" — pulsarla no hace nada (el id que lleva, `core.screens._TIMER_SHORTCUT_EMPTY_ID`, no coincide con ninguna tarea/etiqueta real).
...
`core.screens._timer_items` cruza las etiquetas con `RunningTimer` (`get_running_timer()`, 0 o 1 fila garantizado por la base) para marcar `TimerLabel.running`/`started_at` antes de repartirlas — mismo patrón que `_create_items` marcando `Template.has_pending`. Corriendo, `deck.renderer.render_timer` pinta `COLOR_TIMER_RUNNING` (rosa) con el nombre arriba y el tiempo transcurrido debajo entre corchetes (p.ej. `"Lectura\n[00:05]"`), sin emoji — con dos líneas no hace falta recordar cuál está activo, se lee en la propia tecla; parada, `COLOR_TIMER_STOPPED` (turquesa) con el nombre + emoji de siempre, sin tiempo. El tiempo se recalcula en cada repintado a partir de `started_at` (`_format_elapsed`, único cálculo de tiempo del lado cliente en todo el proyecto, limitado a presentación) — nunca un contador que incrementa solo. La misma opción existe en el menú de una tarea (tecla 2 de `TASK_OPTIONS_LAYOUT`, ver [Menú de opciones de un hábito/tarea](#menú-de-opciones-de-un-hábitotarea-mantener-pulsado)): ahí también se pinta, corriendo, el título de la tarea (`running_timer.title`, ya denormalizado — no hace falta cruzar con `Task`) y el tiempo debajo entre corchetes, en vez del texto "Detener cronómetro" y su icono de cuadrado ⏹️, cuando esa tarea es la que está corriendo.
...
- `_timer_sync` (`config.TIMER_SYNC_SECONDS`, 60s) dispara un `refresh_cycle()` completo (refetch real de las seis lecturas + repintado) para corregir esa cuenta local — deriva de reloj, o que otro cliente haya parado/arrancado el cronómetro entre medias — sin esperar a los 15 min de `REFRESH_SECONDS`.
