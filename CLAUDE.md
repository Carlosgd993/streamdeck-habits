...
- **`base.py`** — los **puertos**. Contiene todo lo que el resto del proyecto necesita saber de "la API", sin acoplarse a ningún backend:
  ...
  - **`TaskProvider`** (ABC): `get_tasks() -> list[Task]` (solo pendientes, **ya ordenadas** por el proveedor), `complete_task(task) -> None` y `set_priority(task, priority) -> None` (cambia la prioridad de una ocurrencia pendiente, sin devolver nada, igual que `complete_task`; la usa el menú de opciones de una tarea). Está **separado a propósito** de `HabitProvider`: son dos capacidades distintas y un backend puede ofrecer una sin la otra.
...
El menú de un **hábito** sigue siendo un **prototipo**: solo un par de teclas de aviso ("Opciones de hábito" + "Próximamente"), la maqueta sobre la que montar opciones de verdad (editar, posponer, omitir…) en una sesión futura. El de una **tarea** ya tiene la primera opción real: **cambiar la prioridad** (teclas 11-14). Ver "Cómo extenderlo" más abajo para añadir opciones nuevas a cualquiera de los dos.
...
7. **Cambiar la prioridad de una tarea** (dentro del menú de opciones de una tarea, `PressAction` de tipo `"task_set_priority"`) → el id de la tarea no viaja en el payload (solo la prioridad elegida): se lee de `screen.entry_item_id` en el momento de ejecutar, se reserva con `_claim`/`_release` igual que cualquier otra escritura, y llama a `set_priority(task, priority)`. Éxito → mutación optimista de `task.priority` + `exit_item_options` (vuelve a la vista de origen). Fallo → tecla en rojo con el código, sin salir del menú. Ver [Menú de opciones de un hábito/tarea (mantener pulsado)](#menú-de-opciones-de-un-hábitotarea-mantener-pulsado).
