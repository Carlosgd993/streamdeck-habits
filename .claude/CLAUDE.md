...
- **`habits/`** — jerarquía de tipos de hábito TickTick:
  - `base.py`: `Habit` (ABC) — `id`, `name`, `is_done_today`, `is_locked` (por defecto `False`: la tecla siempre admite pulsaciones), `display_label` (por defecto el nombre a secas), `build_checkin_payload(stamp, current_value=0.0)` abstracto.
